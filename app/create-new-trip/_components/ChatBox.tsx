  ...
  const onSend = async (manualInput?: string) => {
    ...
    try {
      ...
      if (
        ...
        String(error).includes("INSUFFICIENT_CREDITS")
      ) {
    ...
    if (ui === "final") {
      return (
        <FinalUi
          viewTrip={() => {
            if (savedTripId) {
              router.push(`/view-trip/${savedTripId}`);
            }
          }}
          disable={!savedTripId || loading}
        />
      );
    }
            ...
            {msg.role === "user" ? (
              ...
              <div className="flex justify-start mt-2">
                <div className="max-w-lg bg-gray-200 text-black px-4 py-2 rounded-lg">
                  {msg.content}
                  <div className="mt-2">{RenderGenerativeUi(msg.ui ?? "")}</div>
                </div>
              </div>
            )}
          </div>
